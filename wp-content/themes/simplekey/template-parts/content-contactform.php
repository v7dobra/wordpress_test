...
if(isset($_POST['submitted'])) {
		...
		//Check Captcha
		if(isset($SK['enable_captcha']) && $SK['enable_captcha']==1){
		  if(trim($_POST['captcha']) === '') {
			$captchaError = __('You forgot to enter CAPTCHA.','SimpleKey');
			$hasError = true;
		  } else {
			if($_POST['captcha']!==$_SESSION['sk_captcha']) {
...
<?php }else{ ?>
  ...
  <form id="contactForm" method="post" action="?">
   ...
   <label><textarea class="requiredField" name="comments" id="comments" placeholder="<?php _e('Message','SimpleKey');?>"><?php 
    ...
    ?></textarea></label>
    <?php if(isset($SK['enable_captcha']) && $SK['enable_captcha']==1):?>
    <div class="cp">
    <input type="text" name="captcha" id="captcha" value="" class="requiredField captcha" placeholder="<?php _e('Captcha','SimpleKey');?>" /><a href="javascript:refreshCaptcha();" class="cpt-img"><img id="mycaptcha" src="<?php echo get_template_directory_uri();?>/inc/functions/captcha/sk_captcha.php?rand=<?php echo rand();?>" /></a>
    </div>
   <input type="hidden" name="get_captcha" id="get_captcha" value="<?php echo $_SESSION['sk_captcha'];?>" />
   <?php endif;?>
...
var verify='<?php echo get_template_directory_uri();?>/inc/functions/captcha/sk_check.php';
